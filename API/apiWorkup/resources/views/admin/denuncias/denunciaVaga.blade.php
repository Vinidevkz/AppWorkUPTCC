...
<div class="row">
  ...
  <div class="col-9 mt-4">
    ...
    @forelse($denuncias as $denuncia)
        <tr>
            <td>{{ $denuncia->idDenunciaVaga }}</td> <!-- Adicione o ID aqui, se necessário -->
            <td>
            <a href="{{ route('denunciaVaga.show', $denuncia->idDenunciaVaga ) }}" class="visualizar-link">
            {{ $denuncia->usuario->nomeUsuario }}
                  </a>
            </td>
            <td>{{ $denuncia->vaga->nomeVaga ?? 'Não Associado' }}</td> <!-- Atualizado para exibir o nome -->
            <td>{{ $denuncia->status->tipoStatus ?? 'Não Associado' }}</td>
            <td>
            <form action="{{ route('vaga.delete', $denuncia->vaga->idVaga) }}" method="POST" class="d-inline">
                    @csrf
                    @method('DELETE')
                    <button onclick="return confirm('Realmente deseja excluir essa Vaga?')" type="submit" class="btn btn-outline-danger btn-sm"><span class="bi-trash-fill"></span>&nbsp;Bloquear</button>
                  </form>
            </td>
        </tr>
