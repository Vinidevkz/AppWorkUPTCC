...
import { View, Text, Image, ScrollView, SafeAreaView, TouchableOpacity, ActivityIndicator, FlatList, TextInput, Alert } from "react-native";
...
export default function Profile({ navigation }) {
  ...
  const skillsUser = [skill1Usuario?.trim(), skill2Usuario?.trim(), skill3Usuario?.trim(), skill4Usuario?.trim(), skill5Usuario?.trim()].filter((skill) => skill);
  ...
  return (
    <SafeAreaView style={{ backgroundColor: theme.backgroundColor }}>
      <View style={[styles.containerTop, { backgroundColor: theme.backgroundColorNavBar }]}>
        <Text style={[styles.DMSansBold, styles.title, { color: theme.textColor }]}>Meu Currículo</Text>
      ...
      <ScrollView contentContainerStyle={[styles.scrollViewContent, { backgroundColor: theme.backgroundColor }]}>
        <View style={{ width: "100%", backgroundColor: theme.backgroundColor }}>
          <View style={styles.profileBackgroundImageCont}>
            <Image source={dadosUser.fotoBanner ? { uri: dadosUser.fotoBanner } : require("../../assets/icons/profilebgempty.png")} style={styles.profileBackgroundImg} />

            <View style={[styles.profileIconBox, { borderColor: theme.borderColor }]}>
              <Image source={dadosUser.fotoUsuario ? { uri: dadosUser.fotoUsuario } : require("../../assets/icons/manicon.jpg")} style={styles.icon} />
            </View>
          </View>
          ...
          <View style={[styles.profileCont, { backgroundColor: theme.backgroundColor }]}>
            <View style={styles.profileHeader}>
              <View>
                <Text style={[styles.DMSansBold, styles.profileName, { color: theme.textColor }]}>{dadosUser.nomeUsuario || <ActivityIndicator size={"small"} color={"#20dd77"} />}</Text>
                <Text style={[styles.DMSansRegular, styles.profileUserName, { color: theme.textColor }]}>@{dadosUser.usernameUsuario || <ActivityIndicator size={"small"} color={"#20dd77"} />}</Text>
            ...
            <View style={[styles.profileBioCont, { flexDirection: "row", alignItems: "center", justifyContent: "space-between" }]}>
              <View style={{ width: 250 }}>
                <Text style={[styles.DMSansRegular, styles.text, { color: theme.textColor }]}>{dadosUser.sobreUsuario || <ActivityIndicator size={"small"} color={"#20dd77"} />}</Text>
              </View>

              <View style={{ borderRadius: 20, borderWidth: 2, borderColor: "#20dd77", padding: 10, marginRight: 10 }}>
                <Text style={[styles.DMSansRegular, styles.text, { color: theme.textColor }]}>#{dadosUser.areaInteresseUsuario || <ActivityIndicator size={"small"} color={"#20dd77"} />}</Text>
              </View>
            </View>
            ...
            >
              <View style={styles.profileSkillsCont}>
                <Text style={[styles.DMSansBold, styles.title, { color: theme.textColor, fontSize: 19, borderBottomWidth: 1, borderColor: "#c4c4c4" }]}>Formação Acadêmica:</Text>
                <Text style={[styles.DMSansRegular, styles.text, { color: theme.textColor }]}>
                  <Text style={styles.DMSansBold}>Ensino Médio:</Text>{" "}
                  {dadosUser.ensinoMedio ? (
                    <Text>
                      {dadosUser.ensinoMedio}, {dadosUser.anoFormacao}
                    </Text>
                  ) : (
                    <Text>Incompleto</Text>
                  )}
                </Text>
                <Text style={[styles.DMSansRegular, styles.text, { color: theme.textColor }]}>
              ...
              <View style={styles.profileLinksCont}>
                <Text style={[styles.DMSansBold, styles.title, { color: theme.textColor, fontSize: 19, borderBottomWidth: 1, borderColor: "#c4c4c4" }]}>Contatos:</Text>
                <View style={styles.linkLine}>
                  <Text style={[styles.DMSansBold, styles.linkTitle, { color: theme.textColor }]}>Telefone: </Text>
                  <Text style={[styles.DMSansRegular, styles.linkText, { color: theme.textColor }]}>{dadosUser?.contatoUsuario || <ActivityIndicator size={"small"} color={"#20dd77"} />}</Text>
                </View>
                <View style={styles.linkLine}>
                  <Text style={[styles.DMSansBold, styles.linkTitle, { color: theme.textColor }]}>Email: </Text>
                  <Text style={[styles.DMSansRegular, styles.linkText, { color: theme.textColor }]}>{dadosUser?.emailContato || <ActivityIndicator size={"small"} color={"#20dd77"} />}</Text>
                </View>
              </View>
            </View>
                ...
                <View style={{ gap: 10 }}>
                  ...
                  <View style={{ gap: 15 }}>
                    <View style={[{ alignItems: "center", flexDirection: "row", width: 280, gap: 10 }]}>
                      ...
                      <TextInput placeholder="escreva aqui sua habilidade" placeholderTextColor={theme.placeholderColor} maxLength={50} style={[{ backgroundColor: theme.backgroundColor, borderRadius: 20, width: "100%", paddingHorizontal: 10, color: theme.textColor }]} value={skill1Usuario} onChangeText={(text) => setSkill1Usuario(text)} />
                    </View>

                    <View style={[{ alignItems: "center", flexDirection: "row", width: 280, gap: 10 }]}>
                      <Text style={[styles.DMSansRegular, { color: theme.textColor }]}>Hab. 2: </Text>

                      <TextInput placeholder="escreva aqui sua habilidade" placeholderTextColor={theme.placeholderColor} maxLength={50} style={[{ backgroundColor: theme.backgroundColor, borderRadius: 20, width: "100%", paddingHorizontal: 10, color: theme.textColor }]} value={skill2Usuario} onChangeText={(text) => setSkill2Usuario(text)} />
                    </View>

                    <View style={[{ alignItems: "center", flexDirection: "row", width: 280, gap: 10 }]}>
                      <Text style={[styles.DMSansRegular, { color: theme.textColor }]}>Hab. 3: </Text>

                      <TextInput placeholder="escreva aqui sua habilidade" placeholderTextColor={theme.placeholderColor} maxLength={50} style={[{ backgroundColor: theme.backgroundColor, borderRadius: 20, width: "100%", paddingHorizontal: 10, color: theme.textColor }]} value={skill3Usuario} onChangeText={(text) => setSkill3Usuario(text)} />
                    </View>

                    <View style={[{ alignItems: "center", flexDirection: "row", width: 280, gap: 10 }]}>
                      <Text style={[styles.DMSansRegular, { color: theme.textColor }]}>Hab. 4: </Text>

                      <TextInput placeholder="escreva aqui sua habilidade" placeholderTextColor={theme.placeholderColor} maxLength={50} style={[{ backgroundColor: theme.backgroundColor, borderRadius: 20, width: "100%", paddingHorizontal: 10, color: theme.textColor }]} value={skill4Usuario} onChangeText={(text) => setSkill4Usuario(text)} />
                    </View>

                    <View style={[{ alignItems: "center", flexDirection: "row", width: 280, gap: 10 }]}>
                      <Text style={[styles.DMSansRegular, { color: theme.textColor }]}>Hab. 5: </Text>

                      <TextInput placeholder="escreva aqui sua habilidade" placeholderTextColor={theme.placeholderColor} maxLength={50} style={[{ backgroundColor: theme.backgroundColor, borderRadius: 20, width: "100%", paddingHorizontal: 10, color: theme.textColor }]} value={skill5Usuario} onChangeText={(text) => setSkill5Usuario(text)} />
                    </View>
                  </View>
                </View>
