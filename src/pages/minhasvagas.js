      ...
      >
        ...
        {dadosUser.length > 0 && (
          <View style={{margin: 10}}>
            {dadosUser.map(vaga => (
              <View style={{backgroundColor: theme.backgroundColorNavBar, marginVertical: 10, padding: 20, borderRadius: 15, elevation: 3}}>
                <Text key={vaga.id} style={{color: theme.textColor, fontFamily: 'DMSans-Bold', fontSize: 18}}>{vaga.vaga.nomeVaga}</Text>
                <Text style={{color: theme.textColor, fontFamily: 'DMSans-Bold', fontSize: 15}}>{vaga.vaga.cidadeVaga} • {vaga.vaga.estadoVaga}</Text>
                <Text>{vaga.vaga.nomeEmpresa}</Text>
              </View>
            ))
            }
          </View>
